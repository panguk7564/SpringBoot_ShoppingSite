...
__& 프로젝트명의 의미와 개발 의의__
 ...
 노인과 바다(소설)에서 담긴 가치중 인생의 의미의 측면에서 약하고 의미없는 움직임 같지만 그 움직임과 노력은 그 자체로 의미가 있다. 라는 가치를 참고하여 서로의 가치와 정보를 소통하고 활동할 수 있는 플랫폼을 지향하고자  '노인과 바다' 라는 명칭을 사용하였습니다.
...
- 1.0.00 : (12/5) 상품, 옵션 테이블 형성및 기본 CRUD 매서드 구현

- 1.0.10 : (12/6) 장바구니, 유저 테이블 형성 및 CRUD 일부 구현

- 1.0.15 : (12/7) 주문 테이블 형성 및 기본적인 로그인 구현

- 1.0.20 : (12/8) 로그인 토큰 발급 구현, 클라이언트 로직 개발중(헤더 첨부)

- 1.0.25 : (12/8) 세션 로그인 구현, 장바구니 CRUD 구현, 유저 인증인가 관련 정보 수정

- 1.0.30 : (12/9) 유저 테이블 컬럼추가, 뷰 작업 시작

- 1.0.45 : (12/10) 제품 상세 페이지 구현

- 1.0.50 : (12/11) 회원가입 구현

- 1.0.55 : (12/12) 파일 첨부기능 구현

- 1.0.60 : (12/13) 인증인가 이후 상품등록 + 뷰에 표시 기능 구현, 유저 CRUD 버그 수정

- 1.0.65 : (12/14) 유저 + 상품 CRUD구현 및 연관관계 맵핑완료, 비로그인 상품등록 예외처리 및 상품 뷰에서 옵션 출력 구현

- 1.0.70 : (12/15) 옵션 CRUD 구현 완료, 토큰 + 세션 로그인 구현, 로그아웃시 DB에 토큰삭제 기능 구현

- 1.0.70-1 : (12/16~17) 뷰 버그 수정및 코드 최적화

- 1.0.75 : (12/18) 장바구니 상품 CRUD 구현

- 1.1.0 : (12/19) 결제 API 호출 구현, 주문 방식 변경(하나씩 -> 전체상품 결제)

- 1.1.10 : (12/20) 로그인 보안 강화, 주문 예외처리 추가

- 1.1.15 : (12/21) 결제후 주문내역 형성 구현, 주문 테이블과 장바구니 테이블 연관관계 분리 --> 독립된 객체로 유지

- 1.1.20 : (12/22) 주문 내역 페이징 구현, 결제 완료된 장바구니 상품 자동삭제 구현, 구매후 해당상품의 재고수 감소 구현

- 1.1.20-1 : (12/26) 잉여자원 제거 + 최적화

- 1.1.25 : 뷰 업데이트 (가시성, 잉여 요소 제거)
